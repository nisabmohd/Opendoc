                    ...
                    return <div key={item.name + item.slug}>
                        ...
                        <div className="children flex flex-col gap-4">
                            {item.child.map(child => {
                                return child.link ? <Link key={item.slug + child.link} className="text-sm text-gray-800 " href={`/${item.slug}/${child.link}`}>{child.name}</Link> : <span key={item.slug + child.name} className="text-sm text-gray-400 cursor-not-allowed">{child.name}</span>
                            })}
                        </div>
                    </div>
                })
